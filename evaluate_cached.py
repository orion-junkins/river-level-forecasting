...
args = {
    ...
    "trained_model_dir": "trained_models/RNN/Huber/50",
    "num_inferences": 5,
    "forecast_window": 24
}
...
timestamps = get_recent_available_timestamps(aws_dispatcher, 500)
...
for timestamp in timestamps:
    print("trying timestamp: ", timestamp)
    try:
...
for col in merged.columns:
    if col == "level_true":
        ...
        new_columns.append(pd.to_datetime(col, format='%y-%m-%d_%H-%M').strftime('%Y-%m-%d %H:%M:%S+00:00'))
